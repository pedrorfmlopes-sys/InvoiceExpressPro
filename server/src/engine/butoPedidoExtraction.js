/**
 * BUTÖ — Extrator de PEDIDO (Confirmação de Encomenda)
 * Documento identificado por: BUTO + PEDIDO
...
// Extract a clean SKU from the start of a string (can be 3-12 uppercase alphanum)
function extractSku(str) {
    const m = str.match(/^([A-Z][A-Z0-9]{2,11})(?:\s|$)/);
    return m ? { sku: m[1], rest: str.substring(m[0].length).trim() } : { sku: str, rest: '' };
}

async function processPedido(pdfBuffer) {
    let rawText;
    try {
    ...
    const result = {
        ...
        lines: [],
        docRefs: { linkedPresupuesto: null, linkedInvoice: null },
        shippingMarks: null
    };
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (collectingRef) {
            const rp = l.trim();
            if (rp && !/UD\.|COD\.|DESCRIP|GRACIAS|Total|Fecha/i.test(rp) && /^[A-ZÁÉÍÓÚÑ]/.test(rp) && rp.length < 50) refClienteLines.push(rp);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (firstIsNum) {
            ...
            // Smart SKU + desc extraction (handles long SKUs that have no 3-space gap)
            const rawSecond = cols[1] || '';
            const skuMatch = rawSecond.match(/^([A-Z][A-Z0-9]{2,11})(?:\s|$)/);
            const sku = skuMatch ? skuMatch[1] : rawSecond;
            const descFromSku = skuMatch ? rawSecond.substring(skuMatch[0].length).trim() : '';
            const desc = descFromSku || cols[2] || '';
            const restCols = descFromSku ? cols.slice(2) : cols.slice(3);

            let detailCode = null, finishCode = null, basePrice = 0;
            let incrementPercent = 0, totalBeforeDto = 0, discountPercent = 0, total = 0;

            const textCols = restCols.filter(c => !/^[\d.,]+%?$/.test(c.replace('%', '')) && c);
            const rawNums = restCols.filter(c => /^[\d.,]+%?$/.test(c));
