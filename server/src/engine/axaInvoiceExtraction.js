...
async function processInvoice(pdfBuffer) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (state === 'SEARCHING_START') {
            if (line.includes('Code & Product description') && line.includes('Qty')) {
                state = 'IN_TABLE';
                continue;
            }
            ...
            if (cleanLine.match(/^Rif\. \(DT\)/) || cleanLine.match(/^Ord\. n\./) || cleanLine.match(/^Ref\. est\./) || cleanLine.match(/^Spedizione n\./) || cleanLine === '.' || cleanLine.includes('STOCK ORDER')) continue;

            const cols = cleanLine.split(/\s{2,}/);

            // A typical valid invoice item line:
            // "0201001", "GLOMP WALL HUNG WC...", "PZ", "23,000", "99,00000", "2.277,00", "NI41"
            // With discount: "XX", "YY", "PZ", "1", "100 -5", "95", "NI41"
            if (cols.length >= 5) {
                let sku = cols[0];
                let desc = cols[1];
                let um = '';
                ...
                if (pzIndex !== -1 && pzIndex < cols.length - 1) {
                    ...
                    if (remainingCols.length >= 3) {
                        ...
                        if (pdMatch) {
                            ...
                            discountRaw = pdMatch[2].replace(/-/g, '').replace(/\s+/g, '+');
                        } else {
                            unitPriceRaw = priceDiscountStr;
                        }

                        // Check if it says Omaggio / Omaggio imponibile
                        const isOmaggio = remainingCols.some(c => c.toLowerCase().includes('omaggio'));
                        if (isOmaggio) {
                            totalRaw = "0";
                        } else {
