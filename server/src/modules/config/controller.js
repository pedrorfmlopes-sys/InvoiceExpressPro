...
exports.getUI = (req, res) => {
    try {
        const project = req.project;
        const data = service.getUIPreferences(project);
        res.json(data);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
exports.updateUI = (req, res) => {
    try {
        const project = req.project;
        // Expects { sidebar: {...}, card: {...} }
        const data = service.saveUIPreferences(project, req.body);
...
exports.resetData = async (req, res) => {
    try {
        await service.resetProjectData(req.project);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
exports.deleteProject = async (req, res) => {
    try {
        // Prevent deleting 'default' if critical? Nah, let user decide.
        // Actually, deleting default might crash things on restart if not recreated. 
        // ProjectService.getContext ensures creation.
