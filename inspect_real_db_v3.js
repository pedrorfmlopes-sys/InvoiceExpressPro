...
db.serialize(() => {
    db.all(`SELECT id, docNumber, rawJson FROM documents WHERE docNumber LIKE ?`, [searchTerm], (err, rows) => {
        if (err) {
        ...
        rows.forEach(row => {
            ...
            try {
                ...
                const sm = data.shippingMarks;
                console.log('Shipping Marks:', sm === undefined ? 'UNDEFINED' : (sm === null ? 'NULL' : sm));
