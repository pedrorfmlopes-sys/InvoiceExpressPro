...
/**
 * NicolazziGoldViewer (Digital Twin Edition)
 * Layout: Vertical Split (Twin-Frame)
 * Persistence: Sandbox-First (Satellite DB)
 */
export default function NicolazziGoldViewer({ doc, onClose, updateRow, onFinalize, onSwitch, mode = 'staging' }) {
    // --- State ---
    ...
    async function loadData() {
        ...
        try {
            ...
            // In Archive mode, we skip satellite check and trust Main DB
            // In Staging mode, we check Satellite first
            const checkSatellite = mode === 'staging';

            const pSat = checkSatellite ? api.get(`/api/corev2/extraction-data/nicolazzi_proformas/${doc.id}`) : Promise.resolve({ status: 'rejected' });
            ...
            if (pdfRes.status === 'fulfilled') {
                ...
                console.warn("[GoldViewer] PDF Load Failed", pdfRes.reason);
            }

            // 3. Handle Data Hierarchy
            let finalData = null;

            // A. Satellite (Staging Only)
            if (checkSatellite && satRes.status === 'fulfilled' && satRes.value.data &&
                ...
                console.log("[GoldViewer] Loaded from Satellite");
            }

            // B. Main Doc (Archive or Fallback)
            else if (mainDocRes.status === 'fulfilled' && mainDocRes.value.data) {
                // Main Doc endpoint usually returns { ...columns, ...rawJson } merged, or just columns.
                // We typically need rawJson.
                // If mainDocRes.value.data.rawJson exists, use it.
                // If the endpoint merges them, use the root.
                // `Adapter.getDoc` returns { ...raw, ...r }. So the root object IS the data.

                // However, there is a risk of recursive rawJson if not careful.
                // Let's assume the root object is mostly safe, but if rawJson property exists, use that to be sure.
                const backendRow = mainDocRes.value.data;
                const raw = (backendRow.rawJson && typeof backendRow.rawJson === 'object')
                    ? backendRow.rawJson
                    : (typeof backendRow.rawJson === 'string' ? JSON.parse(backendRow.rawJson) : backendRow);

                if (raw && Object.keys(raw).length > 0) {
                    finalData = raw;
                    console.log("[GoldViewer] Loaded from Main Doc");
                }
            }
            ...
            if (!finalData) {
                ...
                if (propData) {
                    finalData = propData;
                    console.log("[GoldViewer] Loaded from Prop Fallback");
                }
            }

            setSatelliteData(normalizeNicolazziData(finalData || {}));

        } catch (err) {
            console.error("[GoldViewer] Critical Load Error:", err);
    ...
    // Helper: Internal normalize (legacy/safety - better to use the shared one)
    function normalizeData(d) { return normalizeNicolazziData(d); }
    ...
    const handleReProcess = async () => {
        ...
        try {
            ...
            const res = await api.post(`/api/corev2/docs/${doc.id}/reprocess?project=${doc.project}`);
            const freshDoc = res.data;

            // Prioritize the fresh rawJson from the reprocess result
    ...
    const saveData = async (newData) => {
        ...
        try {
            setSaving(true);
            if (mode === 'archive') {
                // Direct DB Update (Validation Mode)
                await api.patch(`/api/corev2/docs/${doc.id}?project=${doc.project || 'default'}`, newData);
            } else {
                // Satellite Update (Staging Mode)
                await api.post(`/api/corev2/extraction-data/nicolazzi_proformas/${doc.id}`, newData);
            }

            // Allow parent to update list if needed
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 z-[5000] bg-black/90 flex flex-col font-sans text-xs w-screen h-screen">
            ...
            <div className="flex-1 flex flex-col overflow-hidden relative min-h-0">
                ...
                {/* FRAME A: PDF (Top, Resizable-ish via toggle) */}
                {showPdf && (
                ...
                <div className="h-[calc(70vh-40px)] w-full bg-[#121212] text-gray-300 flex flex-col overflow-hidden min-h-0">
                    ...
                    <div className="p-4 grid grid-cols-4 gap-4 border-b border-[#333] bg-[#1a1a1a]">
                        ...
                        <div className="border border-[#333] rounded p-2 bg-[#151515] relative group hover:border-green-900/50 transition-colors">
                            ...
                            <textarea
                                ...
                                value={data.entities?.shipping?.address || data.entities?.customer?.address || ''}
                        ...
                        <div className="border border-[#333] rounded p-2 bg-[#151515] relative group hover:border-yellow-900/50 transition-colors flex flex-col gap-2">
                            ...
                            <div className="flex justify-between items-center">
                                <span className="text-[10px] text-gray-500 uppercase">Projeto</span>
                                <input className="w-24 bg-[#0f0f0f] border border-[#333] px-1 text-right text-[10px] text-gray-300 rounded focus:border-yellow-500 outline-none"
                                    value={data.customerRef || (Array.isArray(data.docRefs) ? data.docRefs[0] : (data.docRefs?.customerRef || ''))}
                                    onChange={e => saveData({ ...data, customerRef: e.target.value })} />
                            </div>
                        </div>
                    </div>

                    {/* B2. SMART GRID (Scrollable) */}
                    ...
                    <div className="h-16 bg-[#161616] border-t border-[#333] flex items-center justify-between px-6 shadow-2xl z-20">
                        ...
                        <div className="flex items-center gap-8">
                            ...
                            <button className="px-4 py-2 bg-blue-900/40 text-blue-400 border border-blue-800/50 rounded hover:bg-blue-900/60 transition-colors flex items-center gap-2"
                                onClick={() => {
                                    saveData(data);
                            ...
                            <button className={`${mode === 'archive' ? 'bg-gray-700' : 'bg-green-700 hover:bg-green-600'} text-white font-bold px-6 py-2 rounded shadow-lg transition-transform active:scale-95 flex items-center gap-2`}
                                onClick={mode === 'archive' ? onClose : onFinalize}
                            >
                                {mode === 'archive' ? <span>✔ FECHAR</span> : <span>✔ FINALIZAR</span>}
                            </button>
                        </div>
                    </div>
                </div>
...
// Inline CSS for custom scrollbar webkit in this component scope, usually handled by global theme
// but ensure dark scrollbars for this viewer.
const styles = `
