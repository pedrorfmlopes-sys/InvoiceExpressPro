...
// BUTÖ BUDGET VIEWER (Presupuesto) — Tema Indigo/Azul | Nicolazzi-style layout
export default function ButoBudgetViewer({
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 z-[7000] bg-black/95 flex flex-col font-sans text-xs w-screen h-screen overflow-hidden">
            ...
            {/* TOOLBAR */}
            <div className="h-10 bg-[#0d0d1a] border-b border-indigo-900/40 flex items-center justify-between px-4 shrink-0 shadow-lg">
                <div className="flex items-center gap-3">
                    <h2 className="text-gray-300 font-bold tracking-wider flex items-center gap-2">
                        <span className="text-indigo-400 font-black italic uppercase">Butö Proposta</span>
                        <span className="opacity-30">|</span>
                        <span className="font-mono text-indigo-300">{sd.docNumber || 'NOVO DOC'}</span>
                    </h2>
                    {(loading || reprocessing) && <span className="text-indigo-400 animate-pulse font-bold text-[10px]">A PROCESSAR...</span>}
                </div>
                <div className="flex gap-2">
                    <button onClick={handleReProcess} disabled={reprocessing || loading}
                        className="px-3 py-1 bg-indigo-900/30 hover:bg-indigo-700/40 border border-indigo-700/40 text-indigo-300 rounded text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-40">
                        🔄 Releitura
                    </button>
                    <button onClick={() => setShowPdf(v => !v)}
                        className="px-3 py-1 bg-[#111128] border border-indigo-900/30 text-zinc-500 hover:text-indigo-300 rounded text-[10px] font-black uppercase tracking-widest transition-all">
                        {showPdf ? '⊟ PDF' : '⊞ PDF'}
                    </button>
                    <button onClick={onClose}
                        className="px-3 py-1 bg-[#111128] border border-indigo-900/30 text-zinc-600 hover:text-red-400 rounded text-[10px] font-black uppercase tracking-widest transition-all">
                        ✕ Fechar
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {/* PDF TOP */}
                {showPdf && pdfUrl && (
                    <div className="h-[30vh] shrink-0 bg-[#0a0a18] border-b-4 border-indigo-900/30 relative z-10 shadow-lg">
                        <iframe src={pdfUrl} className="w-full h-full border-none" title="PDF Proposta" />
                    </div>
                )}

                {/* DATA PANEL */}
                <div className={`flex flex-col overflow-hidden bg-[#080816] ${showPdf && pdfUrl ? 'h-[calc(70vh-40px)]' : 'flex-1'}`}>

                    {/* ═══ ENTITY HEADER BAR ═══ */}
                    <div className="grid grid-cols-4 gap-0 border-b border-indigo-900/20 bg-[#0a0a1c] shrink-0" style={{ minHeight: '90px' }}>

                        {/* FORNECEDOR */}
                        <div className="border-r border-indigo-900/20 p-3 relative group">
                            <label className="absolute -top-[7px] left-3 bg-[#0a0a1c] px-1 text-[9px] text-zinc-600 font-black uppercase tracking-[0.15em]">Fornecedor / Supplier</label>
                        ...
                        {/* CLIENTE / BILL TO */}
                        <div className="border-r border-indigo-900/20 p-3 relative group hover:bg-indigo-900/5 transition-colors">
                            <label className="absolute -top-[7px] left-3 bg-[#0a0a1c] px-1 text-[9px] text-indigo-600 font-black uppercase tracking-[0.15em]">Cliente / Bill To</label>
                            {inp(cust.name, v => updEntity('customer', 'name', v), 'font-bold text-indigo-300 text-[11px] mb-0.5')}
                        ...
                        {/* ENTREGA / SHIP TO — mirrors customer (Buto has no separate ship-to) */}
                        <div className="border-r border-indigo-900/20 p-3 relative group hover:bg-indigo-900/5 transition-colors">
                            <label className="absolute -top-[7px] left-3 bg-[#0a0a1c] px-1 text-[9px] text-emerald-700 font-black uppercase tracking-[0.15em]">Entrega / Ship To</label>
                        ...
                        <div className="p-3 relative">
                            <label className="absolute -top-[7px] left-3 bg-[#0a0a1c] px-1 text-[9px] text-zinc-600 font-black uppercase tracking-[0.15em]">Proyecto &amp; Meta</label>
                            <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                                <span className="text-[9px] text-zinc-700 font-black uppercase tracking-wider self-center">Doc Nº</span>
                                <input className="bg-[#050510] border border-indigo-900/30 rounded px-2 h-5 text-right font-mono text-indigo-400 font-black text-[10px] outline-none focus:border-indigo-500"
                                ...
                                <input className="bg-[#050510] border border-indigo-900/20 rounded px-2 h-5 text-right font-mono text-zinc-400 text-[10px] outline-none focus:border-indigo-500"
                                ...
                                <input className="bg-[#050510] border border-zinc-900/20 rounded px-2 h-5 text-right text-zinc-500 text-[10px] outline-none focus:border-indigo-400"
                                    value={sd.dates?.deliveryEstimate || sd.fechaSalida || ''} onChange={e => updDate('deliveryEstimate', e.target.value)} />
                            </div>
                            {/* Ref. Cliente — up to 2 lines */}
                            <div className="mt-2 pt-1 border-t border-indigo-900/10">
                                <span className="text-[9px] text-amber-800 font-black uppercase tracking-wider block mb-0.5">Ref. Cliente</span>
                                <textarea
                                    className="w-full bg-[#050510] border border-amber-900/20 rounded px-2 py-0.5 text-amber-400 font-bold text-[10px] outline-none resize-none focus:border-amber-600 uppercase leading-tight"
                                    style={{ height: '30px' }}
                                    value={sd.shippingMarks || sd.customerRef || ''}
                    ...
                    {/* FILTER */}
                    <div className="flex gap-2 px-3 py-1.5 bg-[#070714] border-b border-indigo-900/10 shrink-0">
                        <input placeholder="🔍 Filtrar por código ou descrição..."
                            className="flex-1 bg-[#050510] border border-indigo-900/20 px-2 h-6 text-zinc-400 text-[10px] rounded outline-none focus:border-indigo-700"
                    ...
                    {/* TABLE */}
                    <div className="flex-1 overflow-auto bg-[#050510] custom-scrollbar">
                        <table className="w-full text-left border-collapse table-fixed">
                            <thead className="bg-[#0a0a1c] text-[9px] uppercase text-indigo-900 font-black sticky top-0 z-10 border-b border-indigo-900/20 tracking-widest">
                                <tr>
                                    <th className="p-2 w-8 text-center border-r border-indigo-900/10">#</th>
                                    <th className="p-2 w-12 text-center border-r border-indigo-900/10">UD.</th>
                                    <th className="p-2 w-28 border-r border-indigo-900/10 text-indigo-500">COD.</th>
                                    <th className="p-2 border-r border-indigo-900/10">DESCRIPCIÓN</th>
                                    <th className="p-2 w-20 text-center border-r border-indigo-900/10 text-cyan-900">DETALLE</th>
                                    <th className="p-2 w-16 text-center border-r border-indigo-900/10 text-cyan-800">ACABADO</th>
                                    <th className="p-2 w-22 text-right border-r border-indigo-900/10">PRECIO (€)</th>
                                    <th className="p-2 w-12 text-center border-r border-indigo-900/10 text-orange-800">%INC.</th>
                                    <th className="p-2 w-22 text-right border-r border-indigo-900/10 text-zinc-700">TOTAL s/Dto</th>
                                    <th className="p-2 w-14 text-center border-r border-indigo-900/10 text-red-900">DTO.%</th>
                                    <th className="p-2 w-24 text-right text-indigo-400">SUBTOTAL</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-indigo-900/10">
                                {filteredLines.length > 0 ? filteredLines.map((line, idx) => (
                                    <tr key={idx} className="group hover:bg-indigo-900/[0.07] transition-colors">
                                        <td className="p-2 text-center text-zinc-800 font-mono text-[9px]">{idx + 1}</td>
                                        <td className="p-0 border-r border-indigo-900/10">
                                        ...
                                        <td className="p-0 border-r border-indigo-900/10">
                                            <input className="w-full h-7 bg-transparent px-2 outline-none text-indigo-400 font-mono font-black focus:bg-indigo-500/10 uppercase"
                                        ...
                                        <td className="p-2 text-right font-mono font-black text-indigo-300 tabular-nums">{(line.total ?? 0).toFixed?.(2)} €</td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan="11" className="p-8 text-center opacity-30 italic">Nenhum item encontrado no documento.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="bg-[#050510] border-t border-indigo-900/30 flex items-center justify-between px-6 py-2 shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.5)]">
                        ...
                        <div className="flex items-center gap-8">
                            ...
                            <div className="flex flex-col items-start gap-0.5">
                                ...
                                <span className="font-mono text-zinc-300 font-bold text-base tabular-nums">
                                    {parseFloat(sd.totals?.subtotal || sd.totals?.gross || 0).toFixed(2)} <span className="text-zinc-600 ml-0.5 text-xs">€</span>
                                </span>
                            </div>
                            ...
                            <div className="flex flex-col items-start gap-0.5">
                                ...
                                <div className="flex items-center bg-[#0a0a20] border border-indigo-900/40 rounded px-2 h-7 focus-within:border-indigo-500 transition-all">
                                    <input
                            ...
                            {/* Total Final — Prominent Cyan/Indigo */}
                            <div className="flex flex-col items-start gap-0.5 ml-4">
                                <span className="text-[9px] uppercase font-black text-indigo-500 tracking-widest">Total Proposta</span>
                                <span className="font-mono text-2xl font-black text-indigo-400 tabular-nums drop-shadow-[0_0_15px_rgba(99,102,241,0.4)]">
                                    {parseFloat(sd.totals?.total || 0).toFixed(2)} <span className="text-indigo-600/60 ml-1 text-sm">€</span>
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            {mode === 'staging' && (
                                <button onClick={() => onFinalize && onFinalize(sd)}
                                    className="flex items-center gap-2 px-8 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg shadow-[0_0_20px_rgba(99,102,241,0.4)] transition-all active:scale-95 font-black uppercase text-[11px] tracking-[0.15em] border border-indigo-400/30">
                                    <span>✔</span> Finalizar
                                </button>
                            )}
                        </div>
            ...
            <style dangerouslySetInnerHTML={{ __html: `.custom-scrollbar::-webkit-scrollbar{width:4px;height:4px}.custom-scrollbar::-webkit-scrollbar-track{background:#05050f}.custom-scrollbar::-webkit-scrollbar-thumb{background:#1a1a3a;border-radius:4px}.custom-scrollbar::-webkit-scrollbar-thumb:hover{background:#4f46e5}` }} />
        </div>,
        document.body
    );
}
