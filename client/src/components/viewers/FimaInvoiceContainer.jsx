...
import FimaInvoiceViewer from './FimaInvoiceViewer';
import api from '../../api/apiClient';

/**
 * FimaInvoiceContainer
 * Manages loading, saving, and PDF sync for FIMA Invoices (Faturas).
 */
export default function FimaInvoiceContainer({ doc, onClose, updateRow, onFinalize, mode = 'staging' }) {
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                console.error('[FimaInvoiceContainer] Load Failed:', err);
    ...
    const handleFinalize = async (currentData) => {
        if (!confirm('Confirmar e arquivar esta fatura?')) return;
        try {
            setSaving(true);
            const project = doc.project || 'default';
            // 1. Sync data to DB
