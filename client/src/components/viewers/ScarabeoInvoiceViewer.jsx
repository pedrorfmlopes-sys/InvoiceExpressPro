...
export default function ScarabeoInvoiceViewer({
    doc, data, loading, saving, pdfUrl,
    onDataChange, onSave, onClose, onFinalize, onReconcile, mode
}) {
    ...
    if (loading) {
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90">
                <div className="text-white text-xl animate-pulse">A carregar fatura Scarabeo...</div>
            </div>
        );
    }
    ...
    const safeData = data || { lines: [], metadata: {}, entities: { supplier: {}, customer: {} }, totals: {} };
    ...
    const filteredLines = lines.filter(l => {
        ...
        return (l.sku || '').toLowerCase().includes(term) ||
            (l.description || '').toLowerCase().includes(term);
    });

    const handleLineChange = (idx, field, rawValue) => {
        ...
        if (field === 'quantity' || field === 'unitPrice' || field === 'discountPercent') {
            const qty = parseFloat(line.quantity) || parseFloat(line.qty) || 0;
            const price = parseFloat(line.unitPrice) || parseFloat(line.price) || 0;
        ...
        // Auto-calc global net total based on lines
        const net = newLines.reduce((acc, l) => acc + (parseFloat(l.total) || 0), 0);
        const transport = parseFloat(safeData.totals?.transport || 0);
        const packaging = parseFloat(safeData.totals?.packaging || 0);
        const vat = parseFloat(safeData.totals?.vat || 0);
    ...
    const handleReProcess = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/reprocess/${doc.id}?project=${doc.project || 'default'}`, { method: 'POST' });
            if (!res.ok) throw new Error("A API falhou");
            const freshDoc = await res.json();
            onDataChange(freshDoc.rawJson || {});
    ...
    const updateHeader = (field, val) => {
        onDataChange({ ...safeData, [field]: val, metadata: { ...meta, [field]: val } });
    };
    ...
    const handleReconcile = async () => {
        if (!safeData.shippingMarks && !meta.client_ref) {
            alert("Erro: Não existe marca/referência nesta fatura para ligar à Proposta.");
            return;
        }
        if (!confirm(`Tentar reconciliar com a Proposta nº "${safeData.shippingMarks || meta.client_ref}"?`)) return;

        try {
            setReconciling(true);
            const res = await fetch(`/api/nicolazzi/reconcile/${doc.id}`, { method: 'POST' });
            if (!res.ok) throw new Error("A API falhou");
            ...
            console.error(err);
            alert("Erro ao reconciliar: " + err.message);
        } finally {
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 z-[7000] bg-black/90 flex flex-col font-sans text-xs w-screen h-screen">
            ...
            <div className="h-10 bg-[#1e1e1e] border-b border-[#333] flex items-center justify-between px-4 select-none shrink-0">
                <div className="flex items-center gap-4">
                    <h2 className="text-gray-300 font-bold tracking-wider flex items-center gap-2">
                        <span className="text-yellow-500">SCARABEO INVOICE VIEWER</span>
                        <span className="opacity-30">|</span>
                        {safeData.docNumber || meta.doc_number || 'SEM NÚMERO'}
                    </h2>
                    {saving && <span className="text-[10px] text-blue-400 animate-pulse">A GRAVAR...</span>}
                    {mode === 'archive' && <span className="text-[9px] bg-gray-800 text-gray-300 px-2 py-0.5 rounded border border-gray-600">ARQUIVO</span>}
                </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden relative min-h-0">
                {/* FRAME A: PDF */}
                {showPdf && (
                    <div className="h-[35vh] shrink-0 bg-[#0f0f0f] border-b-4 border-[#111] relative shadow-lg z-10 transition-all duration-300">
                        {pdfUrl ? (
                            <iframe src={pdfUrl} className="w-full h-full border-none" title="PDF Source"></iframe>
                        ) : (
                            <div className="flex items-center justify-center h-full text-gray-600 animate-pulse">PDF não disponível</div>
                        )}
                    </div>
                )}

                {/* FRAME B: DATA COCKPIT */}
                <div className="flex-1 w-full bg-[#121212] text-gray-300 flex flex-col overflow-hidden min-h-0">
                    ...
                    <div className="p-4 grid grid-cols-4 gap-3 border-b border-[#333] bg-[#1a1a1a]">
                        ...
                        <div className="col-span-1 border border-[#333] rounded p-2 bg-[#151515] relative group hover:border-[#555] transition-colors">
                            ...
                            <input className="w-full bg-transparent border-none outline-none font-bold text-gray-200" value={safeData.entities?.supplier?.name || 'SCARABEO CERAMICHE S.R.L.'} disabled />
                            <textarea
                        ...
                        <div className="col-span-1 border border-[#333] rounded p-2 bg-[#151515] relative group hover:border-blue-900/50 transition-colors">
                            ...
                            <div className="flex items-center gap-2 mt-2 pt-2 border-t border-[#333]">
                                ...
                                <input className="w-full bg-transparent border-none outline-none font-bold text-gray-300 placeholder-gray-600 text-[11px]"
                                    value={safeData.entities?.customer?.vat || meta.customer_vat || ''}
                                    onChange={e => updateEntity('customer', 'vat', e.target.value)}
                                    placeholder="NIF..." />
                            </div>
                        </div>
                            ...
                            <div className="flex-1 flex flex-col gap-2 border-r border-[#222] pr-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <input className="flex-1 bg-[#0f0f0f] border border-[#333] px-2 text-right font-mono text-yellow-500 font-bold rounded focus:border-yellow-500 outline-none h-6"
                                        value={safeData.docNumber || meta.doc_number || ''} onChange={e => updateHeader('docNumber', e.target.value)} />
                                </div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <input className="flex-1 bg-[#0f0f0f] border border-[#333] px-2 text-right font-mono text-gray-300 rounded focus:border-yellow-500 outline-none h-6"
                                        value={safeData.date || meta.doc_date || ''} onChange={e => updateHeader('date', e.target.value)} />
                                </div>
                                <div className="flex items-center gap-2">
                            ...
                            <div className="flex-1 flex flex-col gap-2 pl-1">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <input className="flex-1 bg-[#0f0f0f] border border-[#333] px-2 text-right text-[10px] text-yellow-500/80 rounded focus:border-yellow-500 outline-none h-6"
                                        value={safeData.shippingMarks || meta.client_ref || ''}
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-[#222]">
                                    ...
                                    <tr key={idx} className="group hover:bg-[#1a1a1a] transition-colors">
                                        ...
                                        <td className="p-0 border-r border-[#222]">
                                            <input className="w-full h-full bg-transparent px-2 py-2 outline-none text-gray-300 focus:bg-[#222]"
                                                value={line.description || ''} onChange={e => handleLineChange(idx, 'description', e.target.value)} />
                                        </td>
                                        <td className="p-0 border-r border-[#222]">
                                ...
                                {filteredLines.length === 0 && (
                                    <tr>
                                        <td colSpan="8" className="p-12 text-center text-gray-600 italic">
                    ...
                    <div className="h-16 bg-[#161616] border-t border-[#333] flex items-center justify-between px-6 shadow-2xl z-20">
                        ...
                        <div className="flex items-center gap-8">
                            ...
                            <div className="flex flex-col items-end group">
                                ...
                                }} title="Click para calc automático (3%)">Emb. (Edit)</span>
                                <input
                            ...
                            <div className="flex flex-col items-end">
                                ...
                                <span className="font-mono text-xl font-bold text-yellow-500">
                                    {parseFloat(safeData.totals?.gross || safeData.total || 0).toFixed(2)} €
                                </span>
                            </div>
