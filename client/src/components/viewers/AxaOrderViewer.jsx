...
/**
 * AxaOrderViewer (Pure Presenter)
 * Receives data from Container. Only handles UI interaction.
 */
export default function AxaOrderViewer({
    doc,
    data,
    loading,
    saving,
    ...
    onClose,
    onFinalize,
    onReconcile,  // from container — links OC to proposal
            ...
            <div className="h-10 bg-[#1e1e1e] border-b border-[#333] flex items-center justify-between px-4 select-none shrink-0">
                <div className="flex items-center gap-4">
                    <h2 className="text-gray-300 font-bold tracking-wider flex items-center gap-2">
                        <span className="text-yellow-500">AXA VIEWER (C.PEDIDO)</span>
                ...
                <div className="flex gap-2">
                    ...
                    {onReconcile && (
                        <button onClick={onReconcile} disabled={reconciling} className="px-3 py-1 bg-green-900/30 hover:bg-green-800/50 text-green-400 rounded border border-green-800/60 transition-colors flex items-center gap-1">
                            🔗 Ligar à Proposta
                        </button>
                    )}
                    ...
                    <div className="p-4 grid grid-cols-4 gap-3 border-b border-[#333] bg-[#1a1a1a]">
                        ...
                        <div className="col-span-1 border border-[#333] rounded p-2 bg-[#151515] relative group hover:border-blue-900/50 transition-colors">
                            ...
                            <div className="flex items-center gap-2 mt-2 pt-2 border-t border-[#333]">
                                ...
                                <input className="flex-1 bg-transparent border-none outline-none font-bold text-gray-300 placeholder-gray-600 text-[11px]"
                                    value={safeData.entities?.customer?.nif || ''}
                                    onChange={e => updateEntity('customer', 'nif', e.target.value)}
                                    placeholder="NIF..." />
                            </div>
                            <div className="flex items-center gap-2 mt-1">
                                <span className="text-[9px] text-blue-500 font-bold uppercase tracking-wider">Rif. C.</span>
                                <input className="flex-1 bg-transparent border-none outline-none font-bold text-yellow-500 placeholder-gray-600 text-[11px]"
                                    value={safeData.metadata?.customerRef || ''}
                                    onChange={e => {
                                        const meta = safeData.metadata || {};
                                        onDataChange({ ...safeData, metadata: { ...meta, customerRef: e.target.value } });
                                    }}
                                    placeholder="Referência de Cliente..." />
                            </div>
                        </div>
