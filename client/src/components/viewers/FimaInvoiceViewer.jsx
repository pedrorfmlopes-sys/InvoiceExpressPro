...
/** FimaInvoiceViewer — FIMA Factura. Adds IVA column, Docs Origem, and TOTALE DOCUMENTO. */
export default function FimaInvoiceViewer({ doc, data, loading, saving, pdfUrl, onDataChange, onSave, onClose, onFinalize, onReconcile, mode }) {
    ...
    const updateEntity = (t, k, v) => {
        const e = { ...(safeData.entities || {}) }; e[t] = { ...(e[t] || {}), [k]: v };
        ...
        <div className="fixed inset-0 z-[7000] bg-black/90 flex flex-col font-sans text-xs w-screen h-screen">
            ...
            <div className="h-10 bg-[#1e1e1e] border-b border-[#333] flex items-center justify-between px-4 shrink-0">
                <h2 className="text-gray-300 font-bold tracking-wider flex items-center gap-2">
                    ...
                    <span className="text-amber-400">FATURA</span><span className="opacity-30">|</span>
            ...
            <div className="flex-1 flex flex-col overflow-hidden min-h-0 relative">
                ...
                <div className="flex-1 bg-[#121212] text-gray-300 flex flex-col overflow-hidden min-h-0">
                    ...
                    <div className="p-3 grid grid-cols-4 gap-3 border-b border-[#333] bg-[#1a1a1a]">
                        ...
                        {/* Doc Meta */}
                        <div className="col-span-1 border border-[#333] rounded p-2 bg-[#151515] relative">
                            <label className="absolute -top-2 left-2 bg-[#1a1a1a] px-1 text-[9px] text-amber-500 font-bold uppercase">Fatura Meta</label>
                            {[['Nº Fatura', 'doc_number', 'text-amber-400 font-bold'], ['Data', 'doc_date', 'text-gray-300'], ['Cond. Pag.', 'payment_condition', 'text-gray-400'], ['IBAN', 'bank_iban', 'text-cyan-400']].map(([lbl, k, cls]) => (
                        ...
                        {/* Docs Origem */}
                        <div className="col-span-1 border border-[#333] rounded p-2 bg-[#151515] relative">
                            <label className="absolute -top-2 left-2 bg-[#1a1a1a] px-1 text-[9px] text-orange-400 font-bold uppercase">Docs Origem</label>
                            {[['DDT/Refs', 'source_docs', 'text-orange-300'], ['Ref. Cli.', 'client_ref', 'text-yellow-500'], ['Projeto', 'project_note', 'text-blue-300'], ['Porto', 'shipping_terms', 'text-gray-400']].map(([lbl, k, cls]) => (
                                <div key={k} className="flex items-center gap-2 mb-1">
                                    <span className="text-[9px] text-gray-500 uppercase w-16 shrink-0">{lbl}</span>
                                    <input className={`flex-1 bg-[#0f0f0f] border border-[#333] px-2 text-right font-mono rounded outline-none h-5 text-[10px] ${cls}`} value={meta[k] || ''} onChange={e => updateMeta(k, e.target.value)} />
                                </div>
                    ...
                    <div className="px-4 py-1 bg-[#161616] border-b border-[#2a2a2a] flex items-center gap-3">
                        <input className="flex-1 bg-[#0f0f0f] border border-[#2a2a2a] px-2 text-[10px] text-gray-300 rounded outline-none" placeholder="Filtrar SKU / descrição..." value={itemFilter} onChange={e => setItemFilter(e.target.value)} />
                        <span className="text-[9px] text-gray-600">{filtered.length}/{lines.length}</span>
                    </div>

                    {/* LINE TABLE — includes IVA column */}
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-[#1f1f1f] text-[10px] uppercase text-gray-500 font-bold sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="p-2 w-12 text-center border-r border-[#333]">Qtd</th>
                                    <th className="p-2 w-24 text-right border-r border-[#333]">P.Unit</th>
                                    <th className="p-2 w-12 text-center text-red-500 border-r border-[#333]">Desc1%</th>
                                    <th className="p-2 w-12 text-center text-red-400 border-r border-[#333]">Desc2%</th>
                                    <th className="p-2 w-10 text-center text-amber-500 border-r border-[#333]">IVA%</th>
                            ...
                            <tbody className="divide-y divide-[#222]">
                                {filtered.map((line, idx) => (
                                    <tr key={idx} className="group hover:bg-[#1a1a1a]">
                                        ...
                                        <td className="p-0 border-r border-[#222]"><input className="w-full bg-transparent px-1 py-1.5 outline-none text-center text-amber-500 focus:bg-[#222]" value={line.vatRate ?? ''} onChange={e => updateLine(idx, 'vatRate', e.target.value)} /></td>
                                        <td className="p-2 text-right font-mono font-bold text-gray-200 bg-[#151515]">{parseFloat(line.total || 0).toFixed(2)} €</td>
                                    </tr>
                                ))}
                                {filtered.length === 0 && <tr><td colSpan="9" className="p-12 text-center text-gray-600 italic">Nenhum artigo encontrado.</td></tr>}
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="h-14 bg-[#161616] border-t border-[#333] flex items-center justify-between px-6">
                        <div className="flex items-center gap-6">
                            <div className="flex flex-col items-end">
                                <span className="text-[9px] uppercase text-gray-600 font-bold">Imponível</span>
                                <span className="font-mono text-sm text-gray-400">{safeData.totals?.net || '0.00'} €</span>
                            </div>
                            <div className="flex flex-col items-end">
                                <span className="text-[9px] uppercase text-gray-600 font-bold">IVA</span>
                                <span className="font-mono text-sm text-amber-600">{safeData.totals?.vat || '0.00'} €</span>
                            </div>
                            <div className="flex flex-col items-end">
                                <span className="text-[9px] uppercase text-gray-600 font-bold">Total Fatura</span>
                                <span className="font-mono text-xl font-bold text-amber-400">{safeData.totals?.gross || '0.00'} €</span>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            ...
                            {onReconcile && (
                                <button className="px-4 py-2 bg-yellow-900/40 text-yellow-400 border border-yellow-800/50 rounded hover:bg-yellow-900/60 flex items-center gap-1" onClick={onReconcile}>
                                    🔗 Ligar Proposta
                                </button>
                            )}
            ...
            <style dangerouslySetInnerHTML={{ __html: `.custom-scrollbar::-webkit-scrollbar{width:8px;height:8px}.custom-scrollbar::-webkit-scrollbar-track{background:#121212}.custom-scrollbar::-webkit-scrollbar-thumb{background:#333;border-radius:4px}.custom-scrollbar::-webkit-scrollbar-thumb:hover{background:#555}` }} />
        </div>, document.body
    );
}
