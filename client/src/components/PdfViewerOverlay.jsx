      ...
      <div className="viewer__card" onClick={e => e.stopPropagation()}>
        <div className="viewer__toolbar">
          ...
          <div className="viewer__center">
            <button className="btn btn--icon" title="Zoom -" onClick={() => setZoom(z => Math.max(0.5, parseFloat(parseFloat(z - 0.1).toFixed(2))))}><IconZoomOut /></button>
            <span className="viewer__zoom">{Math.round(zoom * 100)}%</span>
            <button className="btn btn--icon" title="Zoom +" onClick={() => setZoom(z => Math.min(3, parseFloat(parseFloat(z + 0.1).toFixed(2))))}><IconZoomIn /></button>
          </div>
