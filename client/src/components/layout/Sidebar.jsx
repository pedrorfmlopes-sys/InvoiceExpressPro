...
export function Sidebar({ tabs, activeTab, onTabChange }) {
    ...
    // State for UI Config
    const [uiConfig, setUiConfig] = useState({ sidebar: { order: [], labels: {} } });

    useEffect(() => {
        api.get('/api/config/ui')
            .then(res => setUiConfig(res.data || { sidebar: { order: [], labels: {} } }))
            .catch(err => console.error("Failed to load UI layout", err));
    }, []);
    ...
    // Sort Primary Tabs
    const primaryTabs = [...primaryTabsRaw].sort((a, b) => {
        const order = uiConfig.sidebar?.order || [];
        const ia = order.indexOf(a.id);
        const ib = order.indexOf(b.id);

        // Items not in list go to end
        if (ia === -1 && ib === -1) return 0; // Maintain original relative order?
        if (ia === -1) return 1;
        if (ib === -1) return -1;
        return ia - ib;
    });
    ...
    const getLabel = (tab) => {
        if (uiConfig.sidebar?.labels?.[tab.id]) return uiConfig.sidebar.labels[tab.id];
        return tab.label;
    };
    ...
    const NavItem = ({ tab }) => {
        ...
        return (
            ...
            >
                ...
                <span className={isActive ? 'font-semibold ml-1' : 'ml-1'}>{getLabel(tab)}</span>
