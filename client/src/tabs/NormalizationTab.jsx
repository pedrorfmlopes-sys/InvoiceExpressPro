...
export default function NormalizationTab({ project }){
  ...
  async function load(){
    const j=await fetch(qp('/api/normalize', project)).then(r=>r.json()).catch(()=>({}))
  ...
  async function add(){ if(!alias.trim()||!canonical.trim()) return; await axios.post(qp('/api/normalize', project),{type,alias,canonical}); setAlias(''); setCanonical(''); load() }
  async function delRow(t,a){ if(!confirm(`Apagar regra: ${a}?`)) return; await axios.delete(qp('/api/normalize', project),{data:{type:t,alias:a}}); load() }
  return (
    <>
      <div className="card">
